...
#define INFOF(...)			__log_func(__FILE__, __LINE__, LogLevel::Fatal, __VA_ARGS__)
    ...
    // upbound 函数是一个用于将数字向上对齐到指定边界的工具函数。它的作用是计算一个大于或等于给定整数 n 的最小整数，并且该整数是 align 的倍数。默认的 align 参数是 32。
...
#define TRT_VERSION_STRING(major, minor, patch, build)   TRT_STR(major) "." TRT_STR(minor) "." TRT_STR(patch) "." TRT_STR(build)
    ...
    vector<string> glob_image_files(const string& directory) {
        ...
        try {
            ...
            files_.reserve(1000);
            cv::glob(directory + "\\*", files_, false);
    ...
    static __global__ void warp_affine_bilinear_and_normalize_plane_kernel(uint8_t* src, int src_line_size, int src_width, int src_height, float* dst, int dst_width, int dst_height,
        ...
        if (src_x <= -1 || src_x >= src_width || src_y <= -1 || src_y >= src_height) {
            ...
            // same to opencv   // 进行双线性插值：计算出最终的RGB颜色值 c0, c1, c2。
            c0 = floorf(w1 * v1[0] + w2 * v2[0] + w3 * v3[0] + w4 * v4[0] + 0.5f);
            c1 = floorf(w1 * v1[1] + w2 * v2[1] + w3 * v3[1] + w4 * v4[1] + 0.5f);
            c2 = floorf(w1 * v1[2] + w2 * v2[2] + w3 * v3[2] + w4 * v4[2] + 0.5f);
        }
        
        if (norm.channel_type == ChannelType::SwapRB) {   // 通道交换：如果 norm.channel_type 是 SwapRB，则交换红色和蓝色通道。
            float t = c2;
            c2 = c0;  c0 = t;
        }

        // 归一化：根据 norm 类型执行归一化操作。如果是 MeanStd 归一化，使用给定的均值和标准差进行处理；如果是 AlphaBeta 归一化，则使用线性变换。
    ...
    /*
    void save_image_after_denormalization(const std::vector<float>& cpu_output, int width, int height) {
        // 定义均值和标准差
        float mean[] = { 0.485f, 0.456f, 0.406f };
        float std[] = { 0.229f, 0.224f, 0.225f };

        // 创建一个与原图大小相同的图像矩阵，用于存储反归一化后的数据
        ...
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < width; ++j) {
                ...
                // 获取归一化后的 R, G, B 值
                float r = cpu_output[index];
                float g = cpu_output[index + width * height];
                float b = cpu_output[index + 2 * width * height];

                // 反归一化并转换为0-255范围的8位整数
                uint8_t R = static_cast<uint8_t>(std::max(0.0f, std::min(255.0f, ((r * std[0]) + mean[0]) * 255.0f)));
                uint8_t G = static_cast<uint8_t>(std::max(0.0f, std::min(255.0f, ((g * std[1]) + mean[1]) * 255.0f)));
                uint8_t B = static_cast<uint8_t>(std::max(0.0f, std::min(255.0f, ((b * std[2]) + mean[2]) * 255.0f)));

                // 设置像素值 (OpenCV 是 BGR 排列)
                image.at<cv::Vec3b>(i, j) = cv::Vec3b(B, G, R);
            }
        }

        // 保存图像为 PNG 格式（你也可以保存为其他格式，如 JPG）
        cv::imwrite("E:\\vs_project\\My_skin_segment\\workspace\\images\\denormalization.jpg", image);
    }
    */
    
    static void warp_affine_bilinear_and_normalize_plane(uint8_t* src, int src_line_size, int src_width, int src_height, float* dst, int dst_width, int dst_height,
        float* matrix_2_3, uint8_t const_value, const Norm& norm, cudaStream_t stream) {
        ...
        /***************将前处理数据从gpu复制到cpu，反归一化并保存为jpg*****************
        std::vector<float> cpu_output(dst_width * dst_height * 3); // // Allocate CPU memory to store the output
        checkCudaRuntime(cudaMemcpy(cpu_output.data(), dst, sizeof(float) * dst_width * dst_height * 3, cudaMemcpyDeviceToHost));   // Copy data from GPU to CPU
        save_image_after_denormalization(cpu_output, dst_width, dst_height);   // denormalization
        *********************************************************************************/
    }
    ...
    *******************************************************************************************/
    ...
    public:
        struct Job {
            ...
            MonopolyAllocator<Tensor>::MonopolyDataPointer mono_tensor;   // 使用独占分配器(MonopolyAllocator)管理张量内存
        ...
        virtual std::vector<std::shared_future<Output>> commits(const std::vector<Input>& inputs) {
            ...
            for (int epoch = 0; epoch < nepoch; ++epoch) {
                ...
                for (int i = begin; i < end; ++i) {
                    ...
                    if (!preprocess(job, inputs[i])) {   // 3.8ms
                        job.pro->set_value(Output());
                    }

                    //float inference_average_time = (timestamp_now_float() - begin_timer);
                    //printf("The times of [Preprocess] for an image is: [image, FPS] => [%.2f ms, %.2f]\n", inference_average_time, 1000 / inference_average_time);

                    results[i] = job.pro->get_future();
                }

                ///////////////////////////////////////////////////////////
                // 这段代码是一个典型的多线程编程中处理共享资源的例子。具体来说，它是在对一个共享队列 jobs_ 进行操作，使用互斥锁来确保线程安全。
                /************
                    在多线程环境中，多个线程可能同时访问或修改共享的资源。为了防止数据竞争（race condition），通常会使用互斥锁（std::mutex）来保护这些共享资源。
                    在这个例子中，jobs_ 是一个共享的任务队列，它可能被多个线程并发访问。使用 std::unique_lock 和 std::mutex 确保了只有一个线程能够在同一时间操作 jobs_ 队列，从而避免了数据竞争。
                *************/
                {
                    std::unique_lock<std::mutex> l(jobs_lock_);  // 这行代码创建了一个 std::unique_lock 对象 l，并将 jobs_lock_ 互斥锁（std::mutex 类型）锁住。
                                                                 // std::unique_lock 是一个管理互斥锁的智能锁（RAII风格），它会在构造时自动获取锁，并在析构时自动释放锁。这确保了即使在函数异常返回的情况下，锁也会被正确释放，避免死锁。
                    for (int i = begin; i < end; ++i) {  
                        jobs_.emplace(std::move(jobs[i]));       // 这个循环将范围 [begin, end) 内的 jobs 中的元素逐个转移到共享队列 jobs_ 中。
                                                                 // std::move(jobs[i]) 表示将 jobs[i] 中的元素移动到 jobs_ 中，而不是复制。这对于性能优化至关重要，特别是在 Job 对象包含大量数据或者持有一些昂贵的资源（如大块内存、文件描述符等）时。
                                                                 // jobs_.emplace(std::move(jobs[i])) 将 jobs[i] 的内容直接构造到 jobs_ 队列中，而不是先创建一个临时对象再拷贝，这样可以减少不必要的拷贝和析构。
                    };
                }
                cond_.notify_one();
            }
    ...
    */
    const char* type_name(Type type) {
        ...
        case Type::Skin: return "skin_segment";
        default: return "Unknow";
        }
    }
    ...
    struct AffineMatrix {
        ...
        /*************  计算步骤*****************
            1. 计算从原始尺寸到目标尺寸的缩放因子 scale_x 和 scale_y。
            2. 选择较小的缩放因子作为最终的缩放比例 scale，以保持图像比例不变。
            3. 计算并填充 i2d 矩阵，该矩阵将原始图像坐标转换为目标图像坐标。
            4. 使用 OpenCV 的 invertAffineTransform 函数求出逆矩阵 d2i，用于将目标图像坐标转换回原始图像坐标。
            i2d = [[scale    0    x1 ],
                  [  0    scale   x2 ]]
        *****************************************/
        void compute(const cv::Size& from, const cv::Size& to) {  // 计算从输入图像大小到目标图像大小的仿射变换矩阵。  from: 810 * 1080;  to: 640 * 640
            float scale_x = to.width / (float)from.width;    // scale_x = 640 / 810 = 0.790
            float scale_y = to.height / (float)from.height;  // scale_y = 640 / 1080 = 0.592
            float scale = std::min(scale_x, scale_y);   // 0.592
            ...
            cv::invertAffineTransform(m2x3_i2d, m2x3_d2i);   // 使用 OpenCV 函数 cv::invertAffineTransform 计算逆矩阵
        }

        cv::Mat i2d_mat() {
            return cv::Mat(2, 3, CV_32F, i2d);  // 返回 i2d 矩阵对应的 OpenCV cv::Mat 对象，方便在 OpenCV 的函数中使用
        }
    };
    ...
    public:
        ...
        virtual bool startup(const string& file, Type type, int gpuid) {
        ...
        void save_image_from_output(float* image_based_output, int width, int height, const std::string& file_name) {
            // 创建一个用于保存图像数据的矩阵，8位单通道图像
            cv::Mat image(height, width, CV_8UC1);

            // 遍历图像像素，反归一化并转换为 8-bit (0-255)
            for (int i = 0; i < height; ++i) {
                for (int j = 0; j < width; ++j) {
                    int index = i * width + j;
                    float pixel_value = image_based_output[index];
                    uint8_t pixel = static_cast<uint8_t>(pixel_value * 255.0f);
        ...
        virtual void worker(promise<bool>& result) override {
            ...
            auto output = engine->tensor("1001");
            ...
            input->resize_single_dim(0, max_batch_size).to_gpu();  // 设置输入张量的第 0 维为 max_batch_size，即批处理的大小。
            ...
            output_array_device.resize(max_batch_size, 1 + 480 * 480).to_gpu();
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size)) {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                    ...
                    auto& mono = job.mono_tensor->data();   // 从 job.mono_tensor 中提取预处理后的图像数据，并复制到 input 张量中。
                    affin_matrix_device.copy_from_gpu(affin_matrix_device.offset(ibatch), mono->get_workspace()->gpu(), 6);
                    input->copy_from_gpu(input->offset(ibatch), mono->gpu(), mono->count());    // 将每个批次的图像数据从 mono->gpu() 中复制到 input 的相应位置。
                ...
                engine->forward(true);
                //float inference_average_time = (timestamp_now_float() - begin_timer);
                //printf("The times of [ Forward ] for an image is: [image, FPS] => [%.2f ms, %.2f]\n", inference_average_time, 1000 / inference_average_time);


                //auto begin_timer_d = timestamp_now_float();
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {

                    auto& job = fetch_jobs[ibatch];
                    float* image_based_output = output->cpu<float>(ibatch);
                    auto& image_based_skin = job.output;
                    int index = 0;
                    holly_skin skin; // 定义一个 holly_skin 对象

                    
                    for (int cy = 0; cy < 480; cy++)
                    {
                        for (int cx = 0; cx < 480; cx++)
                        {
                            int result_value = image_based_output[index] * 255;
                            skin.skin_mask[index] = (result_value < 0) ? 0 : (result_value > 255) ? 255 : result_value;
                            //printf("output => [%f, %f]\n", ModelOutput[out_num][index], ModelOutput[out_num][index] * 255);
                            ++index;
                        }
                    }
                    image_based_skin.emplace_back(skin);
                    
                    //save_image_from_output(image_based_output, 480, 480, "E:\\vs_project\\My_skin_segment\\workspace\\results\\video_images\\denormalization.jpg");
                    
                    job.pro->set_value(image_based_skin);
                    //float inference_average_time_d = (timestamp_now_float() - begin_timer_d);
                    //printf("decode time is ===============> [frame, FPS] : [%.2f ms, %.2f]\n", inference_average_time_d, 1000 / inference_average_time_d);
                }
        ...
        virtual bool preprocess(Job& job, const Mat& image) override {
            ...
            job.additional.compute(image.size(), input_size);  // 调用 compute 函数，该函数通常用于计算从原始图像大小到目标输入尺寸的变换矩阵，这里很可能是一个仿射变换矩阵。

            tensor->set_stream(stream_);
            tensor->resize(1, 3, input_height_, input_width_);

            size_t size_image = image.cols * image.rows * 3;
            size_t size_matrix = upbound(sizeof(job.additional.d2i), 32);  //upbound 函数将这个矩阵的大小对齐到 32 字节的边界。对齐的目的是确保内存访问效率，尤其是在 GPU 上进行矩阵运算时，可以利用对齐带来的内存访问加速。
            ...
            checkCudaRuntime(cudaMemcpyAsync(image_device, image_host, size_image, cudaMemcpyHostToDevice, stream_));  // cudaMemcpyAsync 是异步的，意味着这个操作不会阻塞主机线程，复制操作会在指定的 CUDA 流 stream_ 上执行，允许主机和设备同时进行其他操作。
            ...
            warp_affine_bilinear_and_normalize_plane(image_device, image.cols * 3, image.cols, image.rows, tensor->gpu<float>(), input_width_, input_height_, affine_matrix_device, 114, normalize_, stream_);

            return true;
        }

        virtual vector < shared_future<SkinArray>> commits(const vector<Mat>& images) override {
            return ThreadSafedAsyncInferImpl::commits(images);
        }

        virtual std::shared_future<SkinArray> commit(const Mat& image) override {
    ...
    void image_to_tensor(const cv::Mat& image, shared_ptr<Tensor>& tensor, Type type, int ibatch) {
        ...
        if (type == Type::Skin) {
            float mean[] = { 0.485, 0.456, 0.406 };
            float std[] = { 0.229, 0.224, 0.225 };
            normalize = Norm::mean_std(mean, std, 1 / 255.0f, ChannelType::SwapRB);
        }
        else {
            INFOE("Unsupport type %d", type);
        }
    ...
    shared_ptr<Infer> create_infer(const string& engine_file, Type type, int gpuid) {
        shared_ptr<YoloTRTInferImpl> instance(new YoloTRTInferImpl());
        if (!instance->startup(engine_file, type, gpuid)) {
            instance.reset();
        }
        return instance;
    }
    ...
    bool compile(
        ...
        const std::string& int8_images_folder,   // 如果使用 INT8 模式，这个参数指定用于校准的图像文件夹路径。
        const std::string& int8_entropy_calibrator_cache_file) {  // 如果使用 INT8 模式，这个参数指定 INT8 熵校准文件的缓存文件路径。

        bool hasEntropyCalibrator = false;     // 一个布尔变量，指示是否有现成的熵校准数据。
        vector<uint8_t> entropyCalibratorData; // 用于存储熵校准数据的字节数组。
        vector<string> entropyCalibratorFiles; // 用于存储熵校准文件的路径列表。


        // 这是一个 Lambda 函数，用于将图像加载到 Tensor 中，并同步数据。用于 INT8 校准时处理图像。
        // Lambda 函数是 C++11 引入的一种方便的、简洁的语法形式，用于定义匿名函数（即没有名字的函数）。
        // int current : 当前处理的图像的索引。
        // int count: 总的图像数量。
        // const vector<string>& files: 包含要处理的图像文件名的向量。
        // shared_ptr<Tensor>& tensor: Tensor 的共享指针，用于存储处理后的图像数据。
        ...
        if (mode == Mode::INT8) {   // 检查是否有现成的校准文件
            ...
            else {   // 如果没有现成的校准文件，代码会通过 glob_image_files 函数从指定的图像文件夹中加载用于校准的图像列表：
        ...
        if (mode == Mode::INT8) {   // 在构建 TensorRT 引擎时，代码会根据是否有现成的校准数据来创建 INT8 校准器 Int8EntropyCalibrator：
            ...
            if (hasEntropyCalibrator) {
                ...
                int8Calibrator.reset(new Int8EntropyCalibrator(entropyCalibratorFiles, calibratorDims, int8process));
            }
        ...
        // 4. 生成校准文件并保存
        //    在生成并构建 TensorRT 引擎后，如果没有现成的校准数据，代码会保存新生成的校准数据到指定的路径：
        if (mode == Mode::INT8) {
            if (!hasEntropyCalibrator) {
                if (!int8_entropy_calibrator_cache_file.empty()) {
